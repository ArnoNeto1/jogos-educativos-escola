...
import { Vehicle, MAX_SPEED, setMaxSpeed } from './Vehicle.js';
import { Camera } from './Camera.js';
import { Controls } from './Controls.js';
import { buildTrack, decodeCells, computeSpawnPosition, computeTrackBounds, TRACK_CELLS, TRACK_CELLS_FACIL, TRACK_CELLS_DIFICIL } from './Track.js';
...
async function init() {
	...
	// Dificuldade escolhida na tela de nivel (games/corrida-kart/index.html define isso antes de carregar este modulo).
	// "medio" reproduz a pista original (com decoracoes e caminhoes estacionados), por isso usa customCells=null,
	// exatamente como o comportamento original sem parametro de mapa.
	const dificuldade = window.DIFICULDADE_CORRIDA || 'medio';
	const VELOCIDADE_POR_NIVEL = { facil: 1.1, medio: 1.5, dificil: 1.95 };
	const CELULAS_POR_NIVEL = { facil: TRACK_CELLS_FACIL, dificil: TRACK_CELLS_DIFICIL };

	setMaxSpeed( VELOCIDADE_POR_NIVEL[ dificuldade ] || VELOCIDADE_POR_NIVEL.medio );

	const mapParam = dificuldade; // usado so como chave de recorde separada por nivel (ver LapTimer)
	const customCells = CELULAS_POR_NIVEL[ dificuldade ] || null;
