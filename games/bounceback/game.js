...
function Update()
{
    ...
    if (isStartLevel)
    {
        ...
        if (levelFrame==200)
            level.DrawText('Um jogo da competição JS13k 2019',   new Vector2(8,9.5), 14);
        if (levelFrame==260)
...
{  
    ...
    {
        bigText = 'Fim de Jogo!'
        DrawText('Pressione Esc',mainCanvasSize.x/2, mainCanvasSize.y/2+80, 42);
    }
...
class LevelExit extends MyGameObject
{
    ...
    Render() 
    {
        ...
        { 
            ...
            let text = (this.type == 2)? 'Portal '+warpLevel : 'Contra o Tempo';
            DrawText(text,0,-24,14,'center',1,color,'#000');
            if (this.type == 3 && speedRunBestTime)
                DrawText('Recorde '+FormatTime(speedRunBestTime),0,-36,12,'center',1,color,'#000');
...
tileImage.onload=_=>{
    // aguarda o navegador reportar um tamanho de janela valido antes de
    // iniciar (protege contra abas criadas em segundo plano/ocultas,
    // onde innerWidth/innerHeight podem ser 0 no primeiro frame)
    const tryInit=_=>window.innerWidth&&window.innerHeight ? Init() : requestAnimationFrame(tryInit);
    tryInit();
};
